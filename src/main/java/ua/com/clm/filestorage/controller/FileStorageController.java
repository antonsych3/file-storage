...
import ua.com.clm.filestorage.handler.CustomRequestHandler;
...
public class FileStorageController {
    ...
    private final CustomRequestHandler requestHeader;
    private final FileService fileService;

    @GetMapping("")
    @ResponseBody
    public FilesResponseDto getFiles(@RequestParam(defaultValue = "", required = false) String[] tags,
                                     @RequestParam(defaultValue = "0", required = false) int page,
                                     @RequestParam(defaultValue = "10", required = false) int size) {
        log.info("REQUEST TO GET FILES BY TAG(-S) - {}", tags);
        FilesResponseDto filesResponseDto = fileService.getFilesByTags(tags, page, size);
        log.info("GOT {} FILES FOR REQUEST BY TAG(-S) - {}", filesResponseDto.getTotal(), tags);
    ...
    public BaseFile uploadFile(@RequestBody FileRequestDto fileRequestDto) {
        ...
        requestHeader.checkRequest(fileRequestDto);
